...
export class SPHSimulator {
    ...
    constructor(
        ...
        this.gridBuildBindGroup = device.createBindGroup({
            ...
            entries: [
                {
                    binding: 0,
                    resource: { buffer: this.cellParticleCountBuffer },
                },
                { binding: 1, resource: { buffer: particleCellOffsetBuffer } },
                { binding: 2, resource: { buffer: particleBuffer } },
                {
                    binding: 3,
                    resource: { buffer: root.unwrap(environmentBuffer) },
                },
                {
                    binding: 4,
                    resource: { buffer: root.unwrap(this.sphParamsBuffer) },
                },
            ],
        });
    ...
    execute(commandEncoder: GPUCommandEncoder) {
        ...
        for (let i = 0; i < 2; i++) {
            computePass.setBindGroup(
                0,
                this.root.unwrap(this.gridClearBindGroup)
            );
            computePass.setPipeline(this.gridClearPipeline);
            computePass.dispatchWorkgroups(
                Math.ceil((this.gridCount + 1) / 64)
            );
            ...
            computePass.setBindGroup(
                0,
                this.root.unwrap(this.integrateBindGroup)
            );
    ...
    initDambreak(initHalfBoxSize: number[], numParticles: number) {
        let particlesBuf = new ArrayBuffer(
            sphParticleStructSize * numParticles
        );
        this.numParticles = 0;
